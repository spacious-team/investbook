...
import static ru.investbook.parser.sber.cash_security.SberSecurityDepsitAndWithdrawalTable.SberSecurityDepositAndWithdrawalTableHeader.CODE;
import static ru.investbook.parser.sber.cash_security.SberSecurityDepsitAndWithdrawalTable.SberSecurityDepositAndWithdrawalTableHeader.NAME;
...
public class SberSecuritySecuritiesTable extends AbstractReportTable<Security> {
    ...
    protected SberSecuritySecuritiesTable(BrokerReport report) {
        super(report, "Сделки", FIRST_LINE, null, SberSecurityDepositAndWithdrawalTableHeader.class);
    }

    @Override
    protected Security parseRow(TableRow row) {
        return Security.builder()
                .id(row.getStringCellValue(CODE))
