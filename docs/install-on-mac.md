...
> [!TIP]
> Если вы получили ошибку при попытке открыть файл, то выполните следующие действия:
> 1. Откройте "Системные настройки" (System Settings) -> "Конфиденциальность и безопасность" (Privacy & Security).
> 2. Прокрутите вниз до раздела «Безопасность» (Security).
> 3. Вы увидите сообщение о том, что запуск программы Investbook был заблокирован. Рядом с ним будет кнопка "Открыть всё равно" (Open Anyway).
> 4. Нажмите её и подтвердите действие (Touch ID или пароль).
